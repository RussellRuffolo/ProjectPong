...
func _build_starting_rack() -> void:
	...
	for row in range(4):
		...
		for column in range(cups_in_row):
			...
			cup.scored.connect(_on_cup_scored)
...
func _on_cup_scored(cup: Node3D) -> void:
	if not _attempt_active or _reset_countdown >= 0.0:
		return

	_resolve_attempt(true, cup)
...
func _resolve_attempt(was_score: bool, scored_cup: Node3D) -> void:
	...
	if was_score and scored_cup != null and is_instance_valid(scored_cup):
		...
		if scored_cup.has_method("remove_from_game"):
			scored_cup.call("remove_from_game")
...
func _is_miss() -> bool:
	...
	if _attempt_elapsed >= max_attempt_seconds:
		return true
	if _attempt_elapsed >= settled_after_seconds and _ball.linear_velocity.length() <= settled_speed:
		return true
	return false
