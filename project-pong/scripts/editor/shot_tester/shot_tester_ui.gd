...
func refresh(state: Dictionary) -> void:
	...
	if _release_angle_spin != null:
		_release_angle_spin.value = float(state.get("release_angle_degrees", 42.0))
	...
	for cup_index in _cup_buttons.keys():
		...
		button.modulate = Color(1.0, 1.0, 1.0, 1.0) if is_active else Color(0.45, 0.45, 0.45, 0.75)
	...
	if _collision_label != null:
		_collision_label.text = _format_collision_status(state)
...
func _build_left_panel() -> void:
	...
	_release_angle_spin = _add_spin_row(vbox, "Release Angle", 42.0, 8.0, 88.0, 0.1, _on_release_angle_changed)
...
func _format_collision_status(state: Dictionary) -> String:
	var last_collision: Dictionary = state.get("last_collision", {}) if state.get("last_collision", {}) is Dictionary else {}
	if last_collision.is_empty():
		return "Collision: none yet"
	return "Collision: cup %d | %s | count %d" % [
		int(last_collision.get("cup_index", -1)),
		str(last_collision.get("classification", "")),
		int(state.get("attempt_collision_count", 0)),
	]
