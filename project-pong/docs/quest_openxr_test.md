...
This project currently implements only the basic XR boot scene. It does not include Meta Quest Avatar integration yet because no official Meta avatar SDK/plugin is present in the repository.
...
- The world is intentionally empty except for lighting, sky, and a tiny origin helper.
...
5. Look down near your starting position for the small origin helper.
6. Watch `adb logcat` or Godot's deploy output for `[XR]` startup messages.
...
Meta Quest Avatar support still needs the official Meta/Oculus avatar integration for the selected Godot version, plus Meta developer dashboard configuration, entitlement checks, user authentication, and any required Android permissions. Do not treat this XR scene as avatar-complete.
